...
class GeminiEmbeddingProvider(EmbeddingProvider):
    def __init__(self):
        ...
        self.client = GoogleGenAIEmbeddings(
    ...
    @retry(
        ...
        before_sleep=lambda retry_state: logger.warning(
            f"Gemini Embedding API call failed. Retrying in {retry_state.next_action.sleep} seconds..."
        )
    )
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        if len(embeddings) != len(texts):
            raise ValueError(f"Gemini embedding count mismatch: expected {len(texts)}, got {len(embeddings)}")
        
        for idx, vec in enumerate(embeddings):
            if not vec:
                raise ValueError(f"Empty embedding vector received at index {idx}")
            if any(val is None or val != val for val in vec):
